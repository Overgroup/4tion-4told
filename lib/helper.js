...
exports.processIncomingText = function (phoneNumber, message) {
    ...
    getRandomAnswer(function(random_answer) {
        var question = 'xxx-xxx-' + lastFourDigits + ' Q: ' + message;
        var answer = '? A: ' + random_answer;
        if (answer.length + question.length > 140) {
            question = question.substring(0,139-answer.length);
        }
        var messageForTwitter = question + answer;
        updateTwitterStatus(messageForTwitter);
        sendTextMessage(phoneNumber, random_answer);
    });    
};
